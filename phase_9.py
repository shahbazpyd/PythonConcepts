...
This script demonstrates professional standards: Logging, Testing, Documentation, 
and Style.
"""

"""🧠 The Mental Model: Phase 9
Phase 9 is about moving from "coding in your garage" to "building a skyscraper". 
It's about standards, safety, and collaboration.

Virtual Environments (venv) - The Fenced-Off Construction Site:

Imagine building a house. You don't dump your bricks on the public highway; 
you fence off a specific area.
A Virtual Environment is that fence. It ensures that the tools (libraries) you use 
for Project A don't accidentally mess up Project B.
...
requirements.txt is your shopping list. If you hand this list to another builder 
(pip install -r), they can build an exact replica of your house with the exact 
same materials.

Testing (unittest, pytest) - The Safety Inspector:

Before you let people live in the house, an inspector comes in. They check: "If I flush 
the toilet, does the roof leak?"
Tests are automated inspectors. They run every time you change the code to ensure 
you didn't accidentally break the plumbing while painting the walls.

Documentation (Docstrings) - The User Manual:

You built a complex thermostat. If you don't leave a manual, the homeowner will 
freeze.
Docstrings are the manual written directly on the device, explaining what inputs 
it needs and what it does.

PEP 8 - The Building Code:

There are rules: "Wiring must be blue, pipes must be red."
PEP 8 is the style guide. It ensures that any electrician (developer) who walks 
onto the site later understands the wiring immediately, because it follows the 
standard code.

Logging - The Security Camera:

print() is like shouting "Hey!" in the hallway. It's ephemeral and annoying.
Logging is a security camera recording. It timestamps exactly what 
happened ("Door opened at 10:00 PM"), how severe it was ("Warning: Window cracked"), 
and saves it to a tape (file) for review after a crash."""
import logging
import unittest
...
# We configure the logger to write to the console (or file) with timestamps and 
# severity.
